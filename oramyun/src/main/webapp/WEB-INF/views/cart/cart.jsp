	...
	<html>
<head>
	<title>${member.member_name }님의 장바구니</title>
...
// 	// 비동기식 데이터 요청
// 	$.getJSON("/cart/cart/replyList" + "?n=" + gdsNum, function(data){
// 		var str = "";
...
// 				 + "<div class='userInfo'>"
// 				 + "<span class='userName'>" + this.member_name + "</span>"
...
<div id="root">
	<header id="header">
		<div id="header_box">
<%-- 			<%@ img src="resources/img/bg-img/tes-1.jsp" %> --%>
		</div>
	</header>
...
<%-- 						<img src="${view.gdsImg}"> --%>
					...
					<div class="goodsInfo">
						<p class="gdsName"><span>상품명</span>${"cartList.gdsName"}</p>

<%-- 						<p class="cateName"><span>카테고리</span>${cartList.cateName}</p> --%>

						<p class="gdsPrice">
<%-- 							<span>가격 </span><fmt:formatNumber pattern="###,###,###" value="${cartList.gdsPrice}" /> 원 --%>
						</p>

						<p class="gdsStock">
							<span>재고 </span>${"cartList.gdsStock"}
<%-- 							<fmt:formatNumber pattern="###,###,###" value="${cartList.gdsStock}" /> EA --%>
						</p>

<%-- 						<c:if test="${cartList.gdsStock != 0}"> --%>

						<p class="cartStock">
							<span>구입 수량</span>
							<button type="button" class="plus">+</button>
<%-- 							<input type="number" class="numBox" min="1" max="${cartList.gdsStock}" value="1" readonly="readonly"/> --%>
							<button type="button" class="minus">-</button>

<%-- 							<input type="hidden" value="${cartList.gdsStock}" class="gdsStock_hidden" /> --%>
...
<!-- 				</div> -->
				...
				<div id="reply">
					...
					<c:if test="${member_email == null }">
						<p>후기를 남기시려면 <a href="/login">로그인</a>해주세요</p>
					</c:if>
					...
					<section class="replyForm">
						<form role="form" method="post" autocomplete="off">
							...
							<input type="hidden" name="gdsNum" id="gdsNum" value="${'cartList.gdsNum'}">
